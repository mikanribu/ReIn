...
def test_unknown_provider_raises_clear_error(monkeypatch):
    monkeypatch.setattr(llm_module, "get_settings", lambda: Settings(llm_provider="nope"))
    llm_module.get_chat_model.cache_clear()
    with pytest.raises(RuntimeError, match="Unknown LLM_PROVIDER 'nope'"):
...
def test_anthropic_without_key_raises(monkeypatch):
    monkeypatch.setattr(
        ...
        lambda: Settings(llm_provider="anthropic", anthropic_api_key=None),
    )
    llm_module.get_chat_model.cache_clear()
...
def test_azure_openai_missing_config_raises(monkeypatch):
    ...
    monkeypatch.setattr(
        llm_module, "get_settings",
        lambda: Settings(llm_provider="azure_openai", azure_openai_api_key="k"),
    )
    llm_module.get_chat_model.cache_clear()
