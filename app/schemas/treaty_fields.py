...
class TreatyExtraction(BaseModel):
    ...
    # --- Parties ---
    cedent: ExtractedField = Field(..., description="Name of the ceding company (the reinsured).")
    ...
    attachment_basis: ExtractedField = Field(
        ..., description="'losses_occurring', 'risks_attaching', or 'claims_made'."
    )
    ...
    # --- Commission & profit sharing (proportional) ---
    ...
    # --- Claims ---
    cash_loss_limit: ExtractedField = Field(..., description="Cash loss limit amount (immediate settlement threshold).")
    claims_notification: ExtractedField = Field(
        ..., description="Claims advice/notification threshold and requirements."
    )
    portfolio_transfer: ExtractedField = Field(
        ..., description="Premium/loss portfolio assumption and withdrawal conditions, if any."
    )
